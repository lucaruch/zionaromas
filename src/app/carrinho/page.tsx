  ...
  return (
    <section className="arabic-pattern bg-black pb-20 pt-28 text-white sm:pt-32">
      <div className="container">
        <p className="text-xs uppercase tracking-[0.22em] text-gold">Sua seleção</p>
        <h1 className="mt-3 font-display text-4xl sm:text-5xl">Sacola de compras</h1>
          ...
          <div className="grid min-w-0 gap-4">
            {items.length === 0 ? (
              <div className="border border-gold/20 bg-white/[0.035] p-6 text-center sm:p-10">
                <p className="font-display text-3xl">Sua seleção ainda está vazia</p>
                <p className="mx-auto mt-3 max-w-md text-white/55">Explore as marcas disponíveis e escolha uma fragrância para chamar de assinatura.</p>
                <Link href="/produtos" className="mt-6 inline-flex w-full sm:w-auto">
                  <Button className="w-full sm:w-auto">Explorar perfumes</Button>
                </Link>
              </div>
            ) : (
              items.map((item) => (
