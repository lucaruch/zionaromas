  ...
  return (
    <article
      className={cn(
        ...
        dark
          ? "border border-gold/20 bg-white/[0.035] p-2.5 shadow-[0_24px_70px_rgba(0,0,0,.32)] hover:border-gold/55 sm:p-3"
          : "hover:-translate-y-1"
      )}
    >
      <div className={cn("relative aspect-[4/3.35] overflow-hidden bg-pearl", dark ? "rounded-sm bg-[#f6f2e8]" : "rounded-lg")}>
      ...
      <div className={cn("mt-3 space-y-2.5 sm:mt-4", dark ? "px-1 pb-1" : "")}>
        ...
        <Link href={`/produto/${product.slug}`} className="block">
          <h3 className={cn("line-clamp-2 font-display text-base leading-tight transition sm:text-lg", dark ? "text-white group-hover:text-gold" : "text-black group-hover:text-gold-deep")}>
            {product.name}
          </h3>
        </Link>
        <p className={cn("line-clamp-2 text-xs leading-5 sm:text-sm", dark ? "text-white/55" : "text-black/62")}>{product.shortDescription}</p>
        <div className="flex min-w-0 items-end justify-between gap-3">
          <div className="min-w-0">
            {product.salePrice ? (
              <p className={cn("text-sm line-through", dark ? "text-white/35" : "text-black/40")}>{formatCurrency(product.price)}</p>
            ) : null}
            <p className={cn("text-sm font-semibold sm:text-base", dark ? "text-white" : "text-black")}>
